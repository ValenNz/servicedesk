    ...
    <div class="bg-white rounded-3xl shadow-2xl w-full max-w-md p-8 space-y-6">
      ...
      <form @submit.prevent="handleLogin" class="space-y-4">
        ...
        <div class="flex items-center justify-between">
          ...
          <a href="#" class="text-sm font-semibold text-red-600 hover:text-red-700">Forgot password?</a>
        </div>
      ...
      <div class="pt-4 border-t border-gray-100">
        ...
        <div class="grid grid-cols-2 gap-2 text-xs">
          <button @click="fillDemo('admin')" class="px-3 py-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors text-gray-600 font-medium border border-gray-200">
            Admin
          </button>
          <button @click="fillDemo('employee')" class="px-3 py-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors text-gray-600 font-medium border border-gray-200">
            Employee
          </button>
        </div>
      </div>
...
function fillDemo(type) {
  if (type === 'admin') {
    formData.value.email = 'admin@servicedesk.com'
    formData.value.password = 'password'
  } else if (type === 'employee') {
    formData.value.email = 'employee1@servicedesk.com'
    formData.value.password = 'password'
  }
}
...
async function handleLogin() {
  ...
  try {
    ...
    if (email === 'admin@servicedesk.com' && password === 'password') {
      user = { id: 5, name: 'Admin User', email, role: 'admin' }
    } else if (email === 'employee1@servicedesk.com' && password === 'password') {
      user = { id: 3, name: 'employee', email, role: 'employee' }
    } else if (email === 'employee2@servicedesk.com' && password === 'password') {
      user = { id: 4, name: 'employee 2', email, role: 'employee' }
    }
