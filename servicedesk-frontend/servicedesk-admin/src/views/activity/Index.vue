  ...
  <AppLayout>
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
      <div>
        <h1 class="text-2xl font-bold text-gray-800">
          {{ isAdmin ? 'System Activity Log' : 'My Activity Log' }}
        </h1>
        <p class="text-gray-400 text-sm mt-1">
          {{ isAdmin ? 'Track all system activities and user actions' : 'Track your personal actions and ticket updates' }}
        </p>
      </div>
      <div class="flex gap-2">
        ...
        <button 
          ...
          class="inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-red-50 border border-red-200 text-red-600 rounded-2xl font-semibold text-sm hover:bg-red-100 transition-all"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white rounded-3xl p-4 mb-6 border border-gray-100 shadow-sm">
      <div class="grid grid-cols-1 xl:grid-cols-12 gap-3">
        ...
        <div v-if="isAdmin" class="xl:col-span-3">
          <select 
            v-model="filterCategory" 
            class="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20"
          >
            <option value="">All Users</option>
            <option v-for="user in adminStore.users" :key="user.id" :value="user.name">
              {{ user.name }}
            </option>
          </select>
        </div>
        
        <div :class="isAdmin ? 'xl:col-span-2' : 'xl:col-span-5'">
          <select 
            v-model="filterDate" 
            class="w-full px-3 py-2.5 bg-gray-50 border border-gray-100 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-red-500/20"
          >
...
const visibleActivities = computed(() => {
  ...
  return activities.filter(activity => {
    ...
    if (activity.details?.ticket) {
      const relatedTicket = adminStore.tickets.find(t => t.id === activity.details.ticket)
...
const hasActiveFilters = computed(() => {
  return searchQuery.value || filterStatus.value || (isAdmin.value && filterCategory.value) || filterDate.value !== 'all'
})
...
watch([searchQuery, filterStatus, filterCategory, filterDate, itemsPerPage], () => {
  currentPage.value = 1
})
...
function getActivityIcon(type) {
  const icons = {
    ...
    user_updated: { component: Edit, bg: 'bg-teal-50', color: 'text-teal-600' },
    user_deleted: { component: Trash2, bg: 'bg-red-50', color: 'text-red-600' },
    ...
    user_created: { component: UserPlus, bg: 'bg-teal-50', color: 'text-teal-600' },
    user_updated: { component: Edit, bg: 'bg-teal-50', color: 'text-teal-600' }, 
    user_deleted: { component: Trash2, bg: 'bg-red-50', color: 'text-red-600' }, 
    category_created: { component: FolderKanban, bg: 'bg-indigo-50', color: 'text-indigo-600' },
    category_updated: { component: Edit, bg: 'bg-indigo-50', color: 'text-indigo-600' },
    category_deleted: { component: Trash2, bg: 'bg-red-50', color: 'text-red-600' }
  }
...
function getActivityTypeBadge(type) {
  const badges = {
    ...
    user_updated: 'bg-teal-50 text-teal-700', 
    user_deleted: 'bg-red-50 text-red-700', 
    category_created: 'bg-indigo-50 text-indigo-700',
    category_updated: 'bg-indigo-50 text-indigo-700',
...
function exportLogs() {
  ...
  const rows = filteredData.value.map(activity => {
    ...
    if (activity.details?.user) details.push(`User: ${activity.details.user}`)
  ...
  link.setAttribute('download', `activity-log-${userRole}-${date}.csv`)
